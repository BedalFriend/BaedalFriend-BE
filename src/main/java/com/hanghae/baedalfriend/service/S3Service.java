...
public class S3Service {
    ...
    public List<PhotoDto> uploadImage(List<MultipartFile> multipartFile) {
    ...
    public List<PhotoDto> updateImage(List<MultipartFile> multipartFile, String currentFilePath) {
        List<PhotoDto> photoDtos = new ArrayList<>();
        multipartFile.forEach(file -> {
            String fileName = createFileName(file.getOriginalFilename());
            ObjectMetadata objectMetadata = new ObjectMetadata();
            ...
            //기존 이미지 삭제
            amazonS3.deleteObject(new DeleteObjectRequest(bucket, currentFilePath));
